...
export function BmltHub() {
  ...
  const loadBmltDatabaseData = async () => {
    try {
      ...
      // Verify if admin/author session exists
      try {
        const checkRes = await fetch('/api/admin/me');
        if (checkRes.ok) {
          const authData = await checkRes.json();
          if (authData.authenticated && (authData.admin?.role === 'admin' || authData.admin?.role === 'author')) {
            setIsAdmin(true);
          }
        }
      } catch (e) {
        console.warn('Unable to verify admin status inside MLT desk:', e);
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 md:mt-16">
        ...
        {activeTab === 'cases' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            {/* Control Panel: Add Custom Case Study */}
            <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-zinc-200/85 shadow-sm">
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] text-zinc-500 font-medium">
                  {isAdmin ? 'Add, edit, or delete patient files below.' : 'Clinical scenarios and assessments curated by department faculties.'}
                </p>
              </div>
              {isAdmin ? (
                <button
                ...
                >
                  <Plus size={14} /> {showCaseForm ? 'Collapse Form' : 'Add Custom Case Study'}
                </button>
              ) : (
                <div className="px-3 py-1.5 bg-zinc-50 border border-zinc-200 text-zinc-450 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
                  <FileText size={12} className="text-zinc-450" /> Read-Only Academic Portal
                </div>
              )}
            </div>

            {/* EXPANDABLE CASE FORM */}
            <AnimatePresence>
              {showCaseForm && (
                <motion.div
                  ...
                  className="bg-zinc-50 border-2 border-indigo-200 rounded-[2rem] p-5 sm:p-6 shadow-md overflow-hidden"
                >
                  <form onSubmit={handleSaveCase} className="space-y-4">
                    <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
                      <h4 className="font-display font-black text-base text-indigo-950">
                        {editingCase ? `✎ Edit Case Study: ${editingCase.title}` : '⚕ Create Custom Diagnostic Case Study'}
                      </h4>
                      <span className="text-[10px] bg-indigo-100 font-mono tracking-wider font-extrabold text-indigo-700 px-2 py-0.5 rounded-md">
                        REST Endpoint Activated
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Case Title */}
                      <div className="space-y-1">
                        <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Case Title</label>
                        <input
                          type="text"
                          required
                          value={caseTitle}
                          onChange={(e) => setCaseTitle(e.target.value)}
                          placeholder="e.g., Case of the Jaundiced Cyclist"
                          className="w-full px-3.5 py-2 border border-zinc-200 rounded-xl bg-white text-xs sm:text-sm font-semibold text-zinc-800 outline-none focus:border-black"
                        />
                      </div>

                      {/* Answering Mode Selector */}
                      <div className="space-y-1">
                        <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Answering Mode</label>
                        <select
                          value={caseType}
                          onChange={(e) => setCaseType(e.target.value as any)}
                          className="w-full px-3.5 py-2 border border-zinc-200 rounded-xl bg-white text-xs sm:text-sm font-semibold text-zinc-800 outline-none focus:border-black"
                        >
                          <option value="mcq">Standard Multiple-Choice (MCQ)</option>
                          <option value="paragraph">Clinical Essay/Paragraph Assessment</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Patient Presentation (Symptom & History Report)</label>
                      <textarea
                        required
                    ...
                    {/* Case Core Question */}
                    <div className="space-y-1">
                      <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Clinical / Anatomic Question</label>
                      <input
                        type="text"
                        required
                        value={caseQuestion}
                        onChange={(e) => setCaseQuestion(e.target.value)}
                        placeholder="e.g., Evaluate the clinical indicators. What type of hyperbilirubinemia fits this pathology pathway?"
                        className="w-full px-3.5 py-2 border border-zinc-200 rounded-xl bg-white text-xs sm:text-sm font-semibold text-zinc-800 outline-none focus:border-black"
                      />
                    </div>
                    ...
                    {caseType === 'mcq' ? (
                      <div className="bg-white p-4 rounded-2xl border border-zinc-200/80 space-y-3">
                        <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block font-mono">Multiple Choice Configurations:</span>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <input
                            type="text"
                            required={caseType === 'mcq'}
                            value={caseOptA}
                            onChange={(e) => setCaseOptA(e.target.value)}
                            placeholder="Option A (e.g. Pre-Hepatic hemolytic hyperbilirubinemia)"
                            className="px-3 py-1.5 border border-zinc-200 rounded-lg text-xs font-medium"
                          />
                          <input
                            type="text"
                            required={caseType === 'mcq'}
                            value={caseOptB}
                            onChange={(e) => setCaseOptB(e.target.value)}
                            placeholder="Option B (e.g. Post-Hepatic obstructive hyperbilirubinemia)"
                            className="px-3 py-1.5 border border-zinc-200 rounded-lg text-xs font-medium"
                          />
                          <input
                            type="text"
                            value={caseOptC}
                            onChange={(e) => setCaseOptC(e.target.value)}
                          ...
                          <input
                            type="text"
                            value={caseOptD}
                            onChange={(e) => setCaseOptD(e.target.value)}
                        ...
                        <div className="flex items-center gap-2 pt-2">
                          <span className="text-xs font-bold text-zinc-500">Correct Option Option Key:</span>
                          <select
                            value={caseCorrectOption}
                            onChange={(e) => setCaseCorrectOption(e.target.value)}
                            className="px-2 py-1 border border-zinc-200 rounded bg-white text-xs font-bold"
                          >
                      ...
                      /* Paragraph guidelines options */
                      <div className="bg-white p-4 rounded-2xl border border-zinc-200/80 space-y-1.5">
                        <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide block">
                          Evaluation Target Keywords (Comma-Separated for checking logic)
                        </label>
                        <input
                          type="text"
                          required={caseType === 'paragraph'}
                          value={caseGuidelines}
                          onChange={(e) => setCaseGuidelines(e.target.value)}
                          placeholder="e.g., Obstructive Jaundice, conjugated bilirubin, biliary tract, GFR, creatinine, AKI"
                          className="w-full px-3 py-2 border border-zinc-200 rounded-xl bg-white text-xs font-bold text-zinc-800 outline-none"
                        />
                        <p className="text-[10px] text-zinc-400 font-medium">When students write clinical reports, our evaluation engine checks against these diagnostic keywords to score their work dynamically.</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      <button
                        type="submit"
                        className="px-5 py-2 bg-indigo-650 hover:bg-indigo-900 text-white rounded-xl text-xs font-black uppercase tracking-wider cursor-pointer"
                      >
                        {editingCase ? 'Update Case Study' : 'Save Case Study'}
                      </button>
                      <button
                        type="button"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2.5 lg:space-y-3 lg:gap-0">
                ...
                {isLoadingBmlt ? (
                  <div className="p-5 text-center text-zinc-400 font-mono text-xs">Loading SQLite Patient Files...</div>
                ) : customCases.length === 0 ? (
                  <div className="p-5 text-center text-zinc-400 font-mono text-xs border border-dashed rounded-2xl">No Case Studies in repository. Click "Add Custom Case" to populate!</div>
                ) : (
                  customCases.map((cs) => (
                    <div
                      key={cs.id}
                    ...
                    >
                      ...
                      {/* CRUD Actions in list item */}
                      {isAdmin && (
                        <div className="flex items-center gap-2 pt-2 border-t border-zinc-200/10 z-10">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              openEditCaseForm(cs);
                            }}
                            className={`p-1 rounded hover:bg-white/10 text-xs font-mono font-bold flex items-center gap-0.5 ${
                              selectedCaseId === cs.id ? 'text-zinc-300 hover:text-white' : 'text-zinc-500 hover:text-zinc-800'
                            }`}
                          ...
                          <button
                            ...
                            className="p-1 rounded hover:bg-rose-500/10 text-rose-500 text-xs font-mono font-bold flex items-center gap-0.5"
                            title="Permanently remove case file"
                          >
        ...
        {activeTab === 'practice-mcqs' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            {/* Control Panel: Add Custom MCQ */}
            <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-zinc-200/85 shadow-sm">
              <div className="space-y-0.5">
                <h4 className="text-sm font-black text-zinc-900 uppercase">Interactive Hematology MCQ Library</h4>
                <p className="text-[11px] text-zinc-500 font-medium">Create or modify your own MCQs with set microphotograph image URLs or file anchors.</p>
              </div>
              <button
                onClick={() => {
                  setEditingMcq(null);
                ...
                className="px-4 py-2 bg-indigo-650 hover:bg-zinc-900 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow"
              >
                <Plus size={14} /> {showMcqForm ? 'Collapse Form' : 'Create Custom MCQ'}
              </button>
            </div>
              ...
              {showMcqForm && (
                ...
                >
                  <form onSubmit={handleSaveMcq} className="space-y-4">
                    <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
                      <h4 className="font-display font-black text-base text-indigo-950">
                        {editingMcq ? `✎ Edit Practice MCQ ID #${editingMcq.id}` : '🧪 Define Custom MCQs with Image'}
                      </h4>
                      <span className="text-[10px] bg-indigo-100 font-mono tracking-wider font-extrabold text-indigo-700 px-2 py-0.5 rounded-md">
                        Supports Diagnostic Image URLs
                      </span>
                    </div>

                    {/* Question text */}
                    <div className="space-y-1">
                      <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Question Body Text</label>
                      <input
                        type="text"
                        required
                        value={mcqQuestion}
                        onChange={(e) => setMcqQuestion(e.target.value)}
                        placeholder="e.g., Which cellular abnormality exhibits Howell-Jolly bodies under Wright/Giemsa blood staining?"
                        className="w-full px-3.5 py-2 border border-zinc-200 rounded-xl bg-white text-xs sm:text-sm font-semibold text-zinc-800 outline-none focus:border-black"
                      />
                    </div>

                    {/* Image URL input (Option to set image) */}
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-[11px] font-black text-zinc-500 uppercase tracking-wide">
                        <ImageIcon size={13} className="text-zinc-400" />
                        <span>Question Microphotograph URL (Set image)</span>
                      </div>
                      <input
                        type="url"
                        value={mcqImageUrl}
                        onChange={(e) => setMcqImageUrl(e.target.value)}
                        placeholder="e.g. https://images.unsplash.com/photo-1579161748255-33b3fc535f26?auto=format..."
                        className="w-full px-3.5 py-2 border border-zinc-200 rounded-xl bg-white text-xs sm:text-sm font-semibold text-zinc-800 outline-none focus:border-black"
                      />
                      <p className="text-[10px] text-zinc-400">Provide any public image URL (from Unsplash or medical wikis) to illustrate the cellular pathology.</p>
                    </div>

                    {/* Options list */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-white p-4 rounded-2xl border border-zinc-200/80">
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-zinc-400 uppercase">Option A (Compulsory)</label>
                        <input
                          type="text"
                          required
                          value={mcqOptA}
                          onChange={(e) => setMcqOptA(e.target.value)}
                          placeholder="e.g. Spleen hyperfunction"
                          className="w-full p-2 border border-zinc-200 rounded-lg text-xs font-semibold"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-zinc-400 uppercase">Option B (Compulsory)</label>
                        <input
                          type="text"
                          required
                          value={mcqOptB}
                          onChange={(e) => setMcqOptB(e.target.value)}
                          placeholder="e.g. Severe Hyposplenism / Post-Splenectomy"
                          className="w-full p-2 border border-zinc-200 rounded-lg text-xs font-semibold"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-zinc-400 uppercase">Option C (Optional)</label>
                        <input
                          type="text"
                          value={mcqOptC}
                          onChange={(e) => setMcqOptC(e.target.value)}
                          placeholder="e.g. Acute iron poisoning"
                          className="w-full p-2 border border-zinc-200 rounded-lg text-xs font-semibold"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-zinc-400 uppercase">Option D (Optional)</label>
                        <input
                          type="text"
                          value={mcqOptD}
                          onChange={(e) => setMcqOptD(e.target.value)}
                          placeholder="e.g. Lead intoxication levels"
                          className="w-full p-2 border border-zinc-200 rounded-lg text-xs font-semibold"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Correct Option */}
                      <div className="space-y-1">
                        <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Select Correct Key</label>
                        <select
                          value={mcqCorrect}
                          onChange={(e) => setMcqCorrect(e.target.value)}
                          className="w-full px-3 py-2 border border-zinc-200 rounded-xl bg-white text-xs font-extrabold"
                        >
                          <option value="A">A</option>
                          <option value="B">B</option>
                          <option value="C">C</option>
                      ...
                      {/* Explanation */}
                      <div className="space-y-1">
                        <label className="text-[11px] font-black text-zinc-500 uppercase tracking-wide">Study Guide Explanation</label>
                        <input
                          type="text"
                          value={mcqExplanation}
                          onChange={(e) => setMcqExplanation(e.target.value)}
                          placeholder="e.g., Howell-Jolly bodies represent nuclear remnants left behind under abnormal phagocytic filtration."
                          className="w-full px-3.5 py-2 border border-zinc-200 rounded-xl bg-white text-xs sm:text-sm font-semibold text-zinc-805 outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      <button
                        type="submit"
                        className="px-5 py-2 bg-indigo-650 hover:bg-indigo-900 text-white rounded-xl text-xs font-black uppercase tracking-wider cursor-pointer border-0"
                      >
                        {editingMcq ? 'Update Practice MCQ' : 'Save MCQ to Library'}
                      </button>
                      <button
                        type="button"
                        onClick={() => { setShowMcqForm(false); setEditingMcq(null); }}
                        className="px-4 py-2 bg-white hover:bg-zinc-100 border border-zinc-250 text-zinc-650 rounded-xl text-xs font-bold"
                      >
                        Cancel
                      </button>
              ...
              <div className="lg:col-span-8">
                {isLoadingBmlt ? (
                  ...
                  <div className="bg-white p-12 text-center text-zinc-400 font-mono text-sm border border-dashed rounded-[2rem] space-y-3">
                    ...
                    <button
                      onClick={openNewMcqForm}
                      className="px-4 py-2 bg-indigo-600 hover:bg-zinc-900 text-white rounded-xl text-xs font-black uppercase select-none border-0"
                    >
                      Create First Practice MCQ Now
                    </button>
                  </div>
                ) : (
                  (() => {
                        ...
                        <div className="flex items-center justify-between border-b pb-4">
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="p-1 px-2.5 rounded hover:bg-zinc-100 text-zinc-600 text-xs font-bold flex items-center gap-1 bg-zinc-50 border border-zinc-200"
                            >
                              <Edit size={12} /> Edit
                            </button>
                            <button
                              onClick={() => {
                                handleDeleteMcq(mcq.id);
                              }}
                              className="p-1 px-2.5 rounded hover:bg-rose-50 border border-rose-100 text-rose-600 text-xs font-bold flex items-center gap-1 bg-white"
                            >
